...
class Net(torch.nn.Module):
    ...
    def __init__(self,inputsize,taskcla):
        ...
        self.sizec1 = int(expand_factor*32)

        self.conv2=torch.nn.ModuleList()
        self.sizec2 = int(expand_factor*32)

        self.conv3=torch.nn.ModuleList()
        self.sizec3 = int(expand_factor*64)

        self.conv4=torch.nn.ModuleList()
        self.sizec4 = int(expand_factor*64)

        self.fc1=torch.nn.ModuleList()
        self.sizefc1 = int(expand_factor*512)
        ...
        for j in range(self.M):
            ...
            s=s//2
            self.conv3.append(torch.nn.Conv2d(self.sizec2,self.sizec3,kernel_size=3, padding = 1))
            s=utils.compute_conv_output_size(s,3, padding = 1)
                              
            self.conv4.append(torch.nn.Conv2d(self.sizec3,self.sizec4,kernel_size=3, padding = 1))
            s=utils.compute_conv_output_size(s,3, padding = 1)
            s=s//2
    ...
    def forward(self,x,t,P=None):
        ...
        h=self.relu(self.conv1[P[0,0]](x))
        for j in range(1,self.N):
            h = h + self.relu(self.conv1[P[0,j]](x)) #sum activations

        h_pre=self.drop1(self.maxpool(self.relu(self.conv2[P[1,0]](h))))
        for j in range(1,self.N):
            h_pre = h_pre + self.drop1(self.maxpool(self.relu(self.conv2[P[1,j]](h)))) #sum activations
        h = h_pre

        h_pre=self.relu(self.conv3[P[2,0]](h))
        for j in range(1,self.N):
            h_pre = h_pre + self.relu(self.conv3[P[2,j]](h)) #sum activations
        h=h_pre
        
                              
        h_pre=self.drop2(self.maxpool(self.relu(self.conv4[P[3,0]](h))))
        for j in range(1,self.N):
            h_pre = h_pre + self.drop2(self.maxpool(self.relu(self.conv4[P[3,j]](h)))) #sum activations
        h=h_pre.view(x.size(0),-1)
                              
        h_pre=self.drop2(self.relu(self.fc1[P[4,0]](h)))
        for j in range(1,self.N):
            h_pre = h_pre + self.drop2(self.relu(self.fc1[P[4,j]](h))) #sum activations
        h = h_pre
    ...
    def unfreeze_path(self,t,Path):
        ...
        for i in range(self.M):
            ...
            self.unfreeze_module(self.conv4,i,Path[3,:],self.bestPath[0:t,3,:])
            self.unfreeze_module(self.fc1,i,Path[4,:],self.bestPath[0:t,4,:])
        return
